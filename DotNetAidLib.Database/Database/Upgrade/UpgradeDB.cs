...
namespace DotNetAidLib.Database.Upgrade
{
    public class UpgradeDB
    {
        ...
        public UpgradeDB(IDbConnection cnx)
        {
            if (cnx == null)
                throw new UpgradeException("Connection parameter can't be null.");

            _SQLParser = SQLParserFactory.Instance(cnx);
        ...
        public UpgradeDB(IDbConnection cnx, Stream xmlInputStream)
            : this(cnx)
        {
            _Upgrades = LoadFromXml(xmlInputStream);
        }

        public UpgradeDB(IDbConnection cnx, FileInfo xmlInputFile)
            : this(cnx)
        {
            _Upgrades = LoadFromXml(xmlInputFile);
        }

        public UpgradeDB(IDbConnection cnx, DirectoryInfo baseFolder, String fileNameFilter)
            : this(cnx)
        {
            _Upgrades = LoadFromXml(baseFolder, fileNameFilter);
        }

        public UpgradeDB(IDbConnection cnx, Assembly assembly, String resourcePath)
            : this(cnx)
        {
            _Upgrades = LoadFromXml(assembly, resourcePath);
        }

        public UpgradeDB(IDbConnection cnx, Assembly assembly, String resourceFolder, String fileNameFilter)
            : this(cnx)
        {
        ...
        private IEnumerable<UpgradeGroup> pUpgradeSchema(IDbConnection cnx, String schemaName, Version upgradeTo, string clientUpgradeInfo, bool makeChanges)
        {
            ...
            try
            {
                ...
                bool debugMode = Helpers.Helper.IsCompiledInDebugMode();
