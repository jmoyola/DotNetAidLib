...
{
	public class SQLANSIParser:ISQLParser
	{
		private static ISQLParser _Instance = null;

		private SQLANSIParser() { }
        ...
        public String DataColumnParse(DataColumn dc)
        {
			...
			if (dc.DataType.Equals(typeof(String)))
			{
				if (dc.MaxLength == -1)
					...
					ret += "CHARACTER VARYING(" + dc.MaxLength + ")";
			}
			else if (dc.DataType.Equals(typeof(DateTime)))
				ret += "TIMESTAMP WITHOUT TIME ZONE";
            else if (dc.DataType.Equals(typeof(DateTimeOffset)))
...
where c.CONSTRAINT_SCHEMA = " + schemaName + " AND c.TABLE_NAME = " + tableName +";", false);
        ...
        public SQLUploaderFile GetContentFromFile(IDbConnection cnx, SourceType fileSource, String path) {
            return new SQLANSIUploaderFile(cnx, fileSource, path);
        }

        public String Top(String select, int rowCount) {
            return select.Trim().RegexReplace(@"(^select)", "SELECT TOP " + rowCount, System.Text.RegularExpressions.RegexOptions.IgnoreCase | System.Text.RegularExpressions.RegexOptions.ExplicitCapture);
        }
