...
namespace DotNetAidLib.Core.Process.ServiceDaemon.Imp
{
	...
	{
		private static SystemVServiceHelper _Instance = null;
		public static new SystemVServiceHelper Instance() {
			if (_Instance == null)
        ...
        public override DotNetAidLib.Core.Process.ServiceDaemon.Core.ServiceDaemon Install(String serviceID, String serviceName, String serviceDescription, String executablePath, String executableWorkingDirectory, String runAsUser, IEnumerable<String> servicesDepended, IEnumerable<ServiceTarget> targets, ServiceRestart restart, IEnumerable<String> mountPointsDepended = null)
        {
			...
			FileInfo lockFile = new FileInfo(lockDirectory.FullName + Path.DirectorySeparatorChar + serviceID + ".lock");
            ...
            FileInfo fi=new FileInfo("/etc/init.d/" + serviceID);

			if (fi.Exists)
				fi.Delete ();

            ServiceTarget[] defaultStopTargets = new ServiceTarget[] { ServiceTarget.Halt, ServiceTarget.Reboot, ServiceTarget.SingleUser };
            ServiceTarget[] defaultStartTargets = new ServiceTarget[] { ServiceTarget.MultiUserNonGraphical, ServiceTarget.MultiUserGraphical };
            ...
            if (servicesDepended == null || servicesDepended.Count () == 0)
                servicesDepended = new String [] { "$syslog", "$local_fs", "$network", "$remote_fs", "$named", "$time"};
            ...
            sw.WriteLine ("# Default-Start:     2 4 " + defaultStartTargets.Select(v => (int)v).ToStringJoin(" ")); // 2 4 3 5;
            ...
            else if (appType == ApplicationType.DotNetService)
            {
                ...
                sw.WriteLine("${DAEMON}" + " -n:" + serviceID + " -m:" + serviceID + " - l:\"" + lockFile.FullName + "\" -d:\"" + executableWorkingDirectory + "\" \"" + executablePath + "\"");
            }
