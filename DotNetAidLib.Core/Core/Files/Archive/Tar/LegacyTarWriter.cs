...
namespace DotNetAidLib.Core.IO.Archive.Tar
{
    public class LegacyTarWriter : IDisposable
    {
        ...
        public void WriteDirectory(string directory, bool doRecursive)
        {
            ...
            foreach (var fileName in files) Write(fileName);
        ...
        {
            var path = Path.GetFullPath(file.Name).Replace(Path.GetPathRoot(file.Name), string.Empty);
            ...
            DateTime lastModificationTime, WriteDataDelegate writeDelegate)
        {
            ...
            while (writer.CanWrite) writeDelegate(writer);
        ...
        protected void WriteContent(long count, Stream data)
        {
            ...
            if (count > 0)
            {
                var bytesRead = data.Read(buffer, 0, (int) count);
                if (bytesRead < 0)
                    throw new IOException("LegacyTarWriter unable to read from provided stream");
                if (bytesRead == 0)
                    while (count > 0)
