...
namespace DotNetAidLib.Core.IO.Archive.Tar
{
    internal class TarHeader : ITarHeader
    {
        ...
        public string LastModificationString =>
            Convert.ToString((long) (LastModification - TheEpoch).TotalSeconds, 8).PadLeft(11, '0');
        ...
        public virtual bool UpdateHeaderFromBytes()
        {
            ...
            var unixTimeStamp = Convert.ToInt64(Encoding.ASCII.GetString(buffer, 136, 11), 8);
            ...
            if (storedChecksum == headerChecksum) return true;
...
//            buffer[156] = 20;
        ...
        protected virtual void RecalculateChecksum(byte[] buf)
        {
            ...
            // Calculate checksum
            headerChecksum = 0;
            foreach (var b in buf) headerChecksum += b;
        }
    }
}