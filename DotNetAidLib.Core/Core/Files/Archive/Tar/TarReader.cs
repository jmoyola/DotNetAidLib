...
namespace DotNetAidLib.Core.IO.Archive.Tar
{
    ...
    public class TarReader
    {
        ...
        protected int Read(out byte[] buffer)
        {
            ...
            var bytesRead = inStream.Read(dataBuffer, 0, (int) toRead);
        ...
        public bool MoveNext(bool skipData)
        {
            ...
            var headerRead = inStream.Read(bytes, 0, header.HeaderSize);
            if (headerRead < 512) throw new TarException("Can not read header");
            ...
            if (header.UpdateHeaderFromBytes()) throw new TarException("Checksum check failed");
