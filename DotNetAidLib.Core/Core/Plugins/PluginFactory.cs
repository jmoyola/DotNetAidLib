...
namespace DotNetAidLib.Core.Plugins
{
    ...
    {
        private static PluginFactory<T> _Instance;
        private readonly List<PlugIn<T>> _PlugIns;

        private PluginFactory(FileInfo pluginFile, Type[] constructorParameters)
            : this(new[] {Assembly.LoadFrom(pluginFile.FullName)}, constructorParameters)
        {
        }
        ...
        private PluginFactory(DirectoryInfo baseFolder, bool recursive, string pluginsFilePattern,
        ...
        private PluginFactory(Type[] constructorParameters)
            : this(new FileInfo(Helper.GetEntryAssembly().Location).Directory, true, "*.dll", constructorParameters)
        {
        }

        public IList<PlugIn<T>> PlugIns => _PlugIns;

        public static PluginFactory<T> Instance(DirectoryInfo baseFolder, bool recursive, string pluginsFilePattern,
            Type[] constructorParameters)
        {
            if (_Instance == null)
        ...
        private static IList<Assembly> LoadAssemblies(DirectoryInfo baseFolder, bool recursive,
            string pluginsFilePattern)
        {
            var ret = new List<Assembly>();

            if (recursive)
                foreach (var dir in baseFolder.GetDirectories())
            ...
            try
            {
                ...
                foreach (var tt in asm.GetTypes())
                    ...
                    if (tType.IsAssignableFrom(tt) && !tt.Name.Equals(tType.Name)
                                                   ...
                                                   tt.GetConstructor(constructorParameters) != null)
                        ret.Add(new PlugIn<T>(tt));
            }
            catch (Exception ex)
            {
                Debug.WriteLine("Error loading Types from assembly from '" + assembly.GetName().FullName + "': " +
                                ex.Message);
            }
