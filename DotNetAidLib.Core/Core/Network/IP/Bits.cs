...
namespace DotNetAidLib.Core.Network.IP
{
    public static class Bits
    {
        public static byte[] Not(byte[] bytes)
        {
            ...
            for (var i = 0; i < result.Length; i++) result[i] = (byte) ~result[i];
            return result;
        ...
        public static byte[] And(byte[] A, byte[] B)
        {
            ...
            for (var i = 0; i < A.Length; i++) result[i] &= B[i];
            return result;
        ...
        [Obsolete(
            "This method returns true when A<=B, not A is greater than or equal (>=) B. use LtE method to check A<=B or not.")]
        ...
        [EditorBrowsable(EditorBrowsableState.Never)]
        [Obsolete(
            "This method returns true when A>=B, not A is less than or equal (<=) B. use GtE method to check A>=B or not.")]
        ...
        public static bool LtE(byte[] A, byte[] B, int offset = 0)
        {
            ...
            if (A.Length <= offset || B.Length <= offset)
                throw new ArgumentException("offset must be less than length of A and B.", nameof(offset));
        ...
        public static bool IsEqual(byte[] A, byte[] B)
        {
            ...
            return A.Zip(B, (a, b) => a == b).All(x => x);
        }
        ...
        public static byte[] Increment(byte[] bytes)
        {
            ...
            return bytes
                .Take(incrementIndex)
                .Concat(new[] {(byte) (bytes[incrementIndex] + 1)})
