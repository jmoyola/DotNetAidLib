...
namespace DotNetAidLib.Core.Network.IP
{
...
#endif
    {
        ...
        private static readonly Regex m1_regex =
            new Regex(@"^(?<adr>([\d.]+)|([\da-f:]+(:[\d.]+)?(%\w+)?))[ \t]*/[ \t]*(?<maskLen>\d+)$",
                RegexOptions.IgnoreCase | RegexOptions.Compiled);

        // Pattern 2. Uni address: "127.0.0.1", "::1%eth0"
        private static readonly Regex m2_regex = new Regex(@"^(?<adr>([\d.]+)|([\da-f:]+(:[\d.]+)?(%\w+)?))$",
            RegexOptions.IgnoreCase | RegexOptions.Compiled);

        // Pattern 3. Begin end range: "169.254.0.0-169.254.0.255", "fe80::1%23-fe80::ff%23"
        //            also shortcut notation: "192.168.1.1-7" (IPv4 only)
        private static readonly Regex m3_regex =
            new Regex(
                @"^(?<begin>([\d.]+)|([\da-f:]+(:[\d.]+)?(%\w+)?))[ \t]*[\-–][ \t]*(?<end>([\d.]+)|([\da-f:]+(:[\d.]+)?(%\w+)?))$",
                RegexOptions.IgnoreCase | RegexOptions.Compiled);

        // Pattern 4. Bit mask range: "192.168.0.0/255.255.255.0"
        private static readonly Regex m4_regex =
            new Regex(@"^(?<adr>([\d.]+)|([\da-f:]+(:[\d.]+)?(%\w+)?))[ \t]*/[ \t]*(?<bitmask>[\da-f\.:]+)$",
                RegexOptions.IgnoreCase | RegexOptions.Compiled);
        ...
        public static IPAddressRange Parse(string ipRangeString)
        {
            ...
            string stripScopeId(string ipaddressString)
            {
                return ipaddressString.Split('%')[0];
            }
            ...
            if (m1.Success)
            {
                ...
                baseAdrBytes = Bits.And(baseAdrBytes, maskBytes);
                return new IPAddressRange(new IPAddress(baseAdrBytes),
                    new IPAddress(Bits.Or(baseAdrBytes, Bits.Not(maskBytes))));
            }

            // Pattern 2. Uni address: "127.0.0.1", ":;1"
            var m2 = m2_regex.Match(ipRangeString);
            if (m2.Success) return new IPAddressRange(IPAddress.Parse(stripScopeId(ipRangeString)));
            ...
            if (m3.Success)
            {
                ...
                return new IPAddressRange(
                    IPAddress.Parse(stripScopeId(begin)),
                    IPAddress.Parse(stripScopeId(end)));
            }
        ...
        public int GetPrefixLength()
        {
            ...
            var byteEnd = End.GetAddressBytes();
        ...
        {
            Begin = IPAddress.Parse(TryGetValue(items, nameof(Begin), out var value1)
                ? value1
                : throw new KeyNotFoundException());
            End = IPAddress.Parse(TryGetValue(items, nameof(End), out var value2)
                ? value2
                : throw new KeyNotFoundException());
        }
        ...
        IEnumerable<string> IReadOnlyDictionary<string, string>.Values =>
            GetDictionaryItems().Select(item => item.Value);
        ...
        {
            return GetDictionaryItems().Any(item => item.Key == key);
        }
    ...
    internal class IPv4RangeOperator : IRangeOperator
    {
        ...
        private uint Begin { get; }

        private uint End { get; }
        ...
        public IEnumerator<IPAddress> GetEnumerator()
        {
            for (var adr = Begin; adr <= End; adr++) yield return adr.ToIPv4Address();
        }
        ...
        void ICollection<IPAddress>.CopyTo(IPAddress[] array, int arrayIndex)
        {
            ...
            foreach (var ipAddress in this) array[arrayIndex++] = ipAddress;
        }
    ...
    internal class IPv6RangeOperator : IRangeOperator
    {
        ...
        public IEnumerator<IPAddress> GetEnumerator()
        {
            for (var adr = Begin; adr <= End; adr++) yield return adr.ToIPv6Address();
        }
