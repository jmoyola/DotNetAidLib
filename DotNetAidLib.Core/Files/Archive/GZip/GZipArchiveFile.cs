...
namespace DotNetAidLib.Core.IO.Archive.Zip
{
	...
	{
        private GZipStream gzipFileStream=null;
        TarWriter tarWriter = null;
        TarReader tarReader = null;
        ...
        public override void Open(ArchiveOpenMode openMode){
			...
			try{
                ...
                if (openMode.Equals(ArchiveOpenMode.OpenCreate)){
                    gzipFileStream = new GZipStream(new FileStream(this.File.FullName, FileMode.OpenOrCreate), CompressionLevelParse(this.CompressionLevel));
                    tarWriter = new TarWriter(gzipFileStream);
                }
                else if (openMode.Equals(ArchiveOpenMode.OpenRead)){
                    gzipFileStream = new GZipStream(new FileStream(this.File.FullName, FileMode.Open), CompressionLevelParse(this.CompressionLevel));
		...
		public override void Add(FileInfo fileToAdd, DirectoryInfo relativePath){
            ...
            try
            {
                ...
                */
                ...
                tarWriter.Write(fs, fileToAdd.Length, fileToAdd.Name, username, groupname, permissions, fileToAdd.LastWriteTime);
            }
            catch (Exception ex){
		...
		public override IList<ArchivePart> Content(){
			...
			try {
                ...
                while (tarReader.MoveNext(true))
                    ret.Add(new ArchivePart(tarReader.FileInfo.FileName, tarReader.FileInfo.FileName, tarReader.FileInfo.HeaderSize, tarReader.FileInfo.SizeInBytes, tarReader.FileInfo.LastModification));
            ...
            try
            {
                while (tarReader.MoveNext(false)){
                    if (archivePart.FullName == tarReader.FileInfo.FileName){
                        ...
                        FileInfo destFile = new FileInfo(Path.Combine(outDirectory.FullName, archivePart.FullName));
