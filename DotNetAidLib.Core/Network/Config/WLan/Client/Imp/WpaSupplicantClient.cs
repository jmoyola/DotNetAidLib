...
using DotNetAidLib.Core.OperatingSystem.Imp;
...
namespace DotNetAidLib.Core.Network.Config.WLan.Client.Imp
{
	public class WpaSupplicantClient:WLanClient 
	{
		...
		{
			_WpaSupplicantPath = new FileInfo(".").FromPathEnvironmentVariable("wpa_supplicant");
			_WpaClientPath = new FileInfo(".").FromPathEnvironmentVariable("wpa_cli");
		...
		private void Init() {
			...
			if (!wpaSupplicantConfFile.Exists) {
				...
				LinuxOperatingSystem.Instance().AdminTextWrite(wpaSupplicantConfFile, content);
			}
		}

		public void StartService()
		{
			LinuxOperatingSystem.Instance()
			                    .AdminExecute(
				                    _WpaSupplicantPath,
		...
		public String WpaCliCmd(String cmd){
			...
			ret=DotNetAidLib.Core.OperatingSystem.Core.OperatingSystem.Instance()
				   .AdminExecute(_WpaClientPath, arguments);
