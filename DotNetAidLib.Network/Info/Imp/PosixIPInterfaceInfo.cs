...
namespace DotNetAidLib.Core.Network.Info.Imp
{
    public class PosixIPInterfaceInfo:IPInterfaceInfo
    {
        ...
        public override void AddIPInfo(Core.IPInfo ipInfo) {
            ipFile.CmdExecuteSync("addr add " + ipInfo.Address.ToString() + "/" + ipInfo.Netmask.ToNetworkMaskLength() + " dev " + this.NetworkInterfaceInfo.Name);
        }
        public override void RemoveIPInfo(Core.IPInfo ipInfo) {
            ipFile.CmdExecuteSync("addr del " + ipInfo.Address.ToString() + "/" + ipInfo.Netmask.ToNetworkMaskLength() + " dev " + this.NetworkInterfaceInfo.Name);
        }
    }
}
